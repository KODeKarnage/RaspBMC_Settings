...
def load_settings(__settings__,DISTRO):
        ...
        for key in ("nm.wifi.5GOnly","nm.wifi.address","nm.wifi.adhoc","nm.wifi.dhcp","nm.wifi.dns","nm.wifi.gateway","nm.wifi.key","nm.wifi.netmask","nm.wifi.search","nm.wifi.security","nm.wifi.ssid"):
            exec "dict_nm[\"%s\"]=__settings__.getSetting(\"%s\")" % (key,key)
        for key in ("sys.service.ftp","sys.service.ssh","sys.service.samba","sys.service.remote","sys.service.cron","sys.service.tvh", "sys.service.boblight", "sys.service.surveil", "sys.service.vnc", "sys.service.sab"):
        ...
        if DISTRO == "Raspbmc":
            ...
            if __settings__.getSetting("sys.config.freq.manual") == "false":
                ...
                if __settings__.getSetting("sys.config.freq.profile") == "0":
                    dict_sys["sys.config.freq.arm"]=800
                    for key in ("sys.config.freq.core","sys.config.freq.gpu","sys.config.freq.sdram","sys.config.freq.overvolt","sys.config.freq.isp"):
                        exec "dict_sys[\"%s\"]=init_value[\"%s\"]" % (key,key)
                if __settings__.getSetting("sys.config.freq.profile") == "1":
                    dict_sys["sys.config.freq.arm"]=840
                    dict_sys["sys.config.freq.core"]=350
                    dict_sys["sys.config.freq.gpu"]=253
                    for key in ("sys.config.freq.sdram","sys.config.freq.overvolt","sys.config.freq.isp"):
                        exec "dict_sys[\"%s\"]=init_value[\"%s\"]" % (key,key)
                if __settings__.getSetting("sys.config.freq.profile") == "2":
                    ...
                    dict_sys["sys.config.freq.gpu"]=init_value["sys.config.freq.gpu"]
                if __settings__.getSetting("sys.config.freq.profile") == "3":
                    dict_sys["sys.config.freq.arm"]=1000
                    dict_sys["sys.config.freq.core"]=500
                    dict_sys["sys.config.freq.sdram"]=500
                    dict_sys["sys.config.freq.isp"]=450
                    dict_sys["sys.config.freq.overvolt"]=6
                    dict_sys["sys.config.freq.gpu"]=init_value["sys.config.freq.gpu"]
...
def set_sysconfig(config,time_stamp):
    ...
    file_config.set('root','gpu_mem','128')
...
def set_services(services):
    config_file_dict={"sys.service.ftp":"ftp.conf","sys.service.ssh":"ssh.conf","sys.service.samba":"samba.conf","sys.service.xinetd":"xinetd.conf", "sys.service.remote":"eventlircd.conf","sys.service.avahi":"avahi-daemon.conf","sys.service.cron":"cron.conf",
                      ...
                      "sys.service.tvh":"tvheadend.conf", "sys.service.boblight":"boblight.conf", "sys.service.surveil":"surveil.conf", "sys.service.vnc":"vnc.conf", "sys.service.sab":"sabnzbd.conf"}
