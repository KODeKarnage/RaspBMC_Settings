	...
	if DISTRO == "Raspbmc" and os.path.isfile("/home/pi/.bootstatus") and check_service_running("wd") == "true" and not os.path.isfile("/home/pi/.nowarning"):
        ...
        if __firstrun__ == "false" and os.path.isfile("/boot/config.txt") and  DISTRO == "Raspbmc":
            ...
            if config != system_settings:
                import xbmcgui,time
                dialog = xbmcgui.Dialog()
                ret = dialog.yesno(DISTRO, 'A difference between /boot/config.txt and addon settings was found', 'Do you want to update /boot/config.txt from addon?')
                if ret:
                    time_stamp=str(int(round(time.time())))
                    set_sysconfig(system_settings,time_stamp)
                    ret = dialog.yesno(DISTRO, 'Configuration has been updated', 'Do you want to reboot now?')
                    if ret:
